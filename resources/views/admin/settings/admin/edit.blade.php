            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        @endif
                        <form method="POST" id="form" action="{{ route('admins.update', $admin->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="col-md-6 col-12">
                                    <div class="form-group mandatory">
                                        ...
                                        <select name="admin_group_id" id="admin_group_id" class="choices form-select w-100">
                                            <option value="">Admin Group</option>
                                            @foreach ($admin_groups as $admin_group)
                                                <option value="{{ $admin_group->id }}"
                                                    {{ old('admin_group_id', $admin->admin_group_id) == $admin_group->id ? 'selected' : '' }}>
                                    ...
                                    @foreach ($system_tabs as $tab)
                                        ...
                                        <div class="form-check form-switch fw-bold mt-2">
                                            <input class="form-check-input parent-tab" type="checkbox"
                                                ...
                                                value="{{ $tab->slug }}"
                                                {{ in_array($tab->slug, $full_tabs) ? 'checked' : '' }}>
                                        ...
                                        @foreach ($tab->child as $child)
                                            <div class="form-check form-switch ms-4 mt-1">
                                                <input class="form-check-input child-tab" type="checkbox"
                                                    ...
                                                    value="{{ $child->slug }}"
                                                    {{ in_array($child->slug, $full_tabs) ? 'checked' : '' }}>
                                                <label class="form-check-label">
                                                    {{ $child->name }}
                                                </label>
                                            </div>
